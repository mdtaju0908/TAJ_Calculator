 ...
 export default function PrintingCalculator() {
   ...
   const onPrint = () => {
     ...
     if (!win) return;
     win.document.write(
       `<pre style="font-family: monospace; font-size: 14px">${content}</pre>`
     );
     win.document.close();
     win.focus();
     win.print();
   ...
   return (
     <div className="w-full max-w-3xl bg-card border border-border rounded-2xl shadow-xl p-6 space-y-4">
       <div className="flex gap-2">
         ...
         <button
           onClick={addLine}
           className="bg-accent text-accent-foreground rounded-lg px-4 hover:bg-accent/80"
         >
           Add
         </button>
         <button
           onClick={onPrint}
           className="bg-primary text-primary-foreground rounded-lg px-4 hover:bg-primary/90"
         >
           Print
         </button>
       </div>
