...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <motion.aside
        animate={{ width: isLarge ? (isOpen ? 256 : 80) : (isOpen ? 288 : 0) }}
        transition={{ type: "spring", stiffness: 260, damping: 28 }}
        className={clsx(
          "fixed lg:static top-0 left-0 h-screen shrink-0 flex flex-col transition-all duration-300 z-40",
          "backdrop-blur-xl bg-white/5 border-r border-white/10 shadow-2xl",
          isLarge ? (isOpen ? "w-64" : "w-20") : (isOpen ? "w-72" : "w-0 pointer-events-none")
        )}
        aria-label="Sidebar"
      >
        <div className="p-4 flex items-center justify-between border-b border-white/10">
          {isOpen && <h1 className="text-xl font-bold bg-gradient-to-r from-indigo-400 to-blue-400 bg-clip-text text-transparent">TAJ Calc</h1>}
      ...
      {!isLarge && !isOpen && (
        <button
          ...
          className="fixed left-4 bottom-4 z-30 lg:hidden h-12 w-12 rounded-full bg-white/10 text-white backdrop-blur-md border border-white/10 shadow-2xl hover:bg-white/20"
